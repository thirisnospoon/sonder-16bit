...
/*
 * ЗАГОЛОВКИ БЕЗОПАСНОСТИ ДЕРЖАТСЯ ТОЛЬКО ТЕМ, ЧТО ИХ НЕ ТРОГАЛИ.
 *
 * Они выставлены в конфигурации nginx и не проверялись ничем: правка
 * этой конфигурации — а её правили и ради резолвера, и ради сжатия, и
 * ради отказа шлюза — снимала бы их молча. Пропажа CSP не ломает ни
 * одного сценария и не видна на экране; замечают её после того, как
 * чужой скрипт уже выполнился.
 *
 * Проверяется присутствие и СОДЕРЖАНИЕ: заголовок с пустым или
 * ослабленным значением хуже отсутствующего — он выглядит защитой.
 */
test('заголовки безопасности на месте', async ({ page }) => {
  const ответ = await page.goto('/')
  expect(ответ).not.toBeNull()
  const заголовки = ответ!.headers()

  expect(заголовки['x-content-type-options']).toBe('nosniff')
  expect(заголовки['x-frame-options']).toBe('DENY')
  expect(заголовки['referrer-policy']).toBe('strict-origin-when-cross-origin')

  const csp = заголовки['content-security-policy'] ?? ''
  // Три правила, каждое из которых кто-то однажды ослабляет ради
  // удобства: свой источник по умолчанию, никаких сторонних скриптов,
  // никаких фреймов вокруг.
  expect(csp).toContain("default-src 'self'")
  expect(csp).toContain("script-src 'self'")
  expect(csp).toContain("frame-ancestors 'none'")
  // `unsafe-inline` в скриптах сводит CSP к украшению.
  expect(csp).not.toContain('unsafe-inline')
})

test('внутренние ручки наружу не выходят', async ({ page }) => {
  // Актуатор показывает подробности состояния, включая метрики ядра и
  // настройки. Наружу он не выходит вовсе — не «требует пароль», а
  // отвечает так, будто его нет.
  for (const путь of ['/actuator/health', '/actuator/prometheus', '/actuator/env']) {
    const ответ = await page.request.get(путь, { failOnStatusCode: false })
    expect(ответ.status(), `${путь} обязан быть невидим снаружи`).toBe(404)
  }
})
