...
test('onCleanup вне всякой области — отказ, а не молчание', () => {
  // Молчаливое «ничего не произошло» тут хуже отказа: снятие, которое
  // никто не выполнит, живёт до перезагрузки страницы.
  assert.throws(() => onCleanup(() => {}), /вне области/)
})

test('onCleanup работает в области без единого чтения', () => {
  // Так заводится обработчик события: область есть, читателя нет.
  // Привязка снятия к читателю отказывала бы ровно здесь — там, где
  // снятие нужнее всего.
  const log: string[] = []

  const stop = root((dispose) => {
    onCleanup(() => log.push('снято'))
    return dispose
  })

  assert.deepEqual(log, [])
  stop()
  assert.deepEqual(log, ['снято'], 'снятие области не выполнилось')
})
