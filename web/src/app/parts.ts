...
/**
 * Сервер не ответил — и это ТРЕТЬЕ состояние, а не «гость».
 *
 * Сессия различает «ещё не спрашивали», «спросили — никого» и «спросить
 * не удалось» затем, чтобы пропавшая на секунду сеть не показывала
 * форму входа тому, кто никуда не выходил. Различие было заведено и
 * проверено — а на экране не показывалось НИКАК: страница оставалась
 * пустой, с одним заголовком. Снаружи это выглядит сломанной страницей,
 * а не недоступным сервером, и человек идёт перезагружать вкладку
 * вместо того чтобы подождать.
 *
 * Кнопка повтора обязательна. Сессия спрашивает сервер при запуске и
 * после входа с выходом — сама она не переспросит, и вернувшийся сервер
 * остался бы незамеченным до перезагрузки страницы.
 */
export function нетСвязи(почему: string, повторить: () => void): Node {
  return h(
    'div',
    { class: 'карточка', role: 'alert' },
    h('h2', { class: 'заголовок заголовок--малый' }, 'Сервер не отвечает'),
    h(
      'p',
      { class: 'подпись' },
      `Не удалось связаться с сервером: ${почему}. Это не значит, что вы ` +
        'вышли, — связь пропала, а не сессия.',
    ),
    h(
      'div',
      { class: 'приглашение__ряд' },
      h('button', { class: 'кнопка', type: 'button', onClick: повторить },
        'Повторить'),
    ),
  )
}
