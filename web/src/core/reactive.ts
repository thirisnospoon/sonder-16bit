 ...
 *
 * Зовётся ИЗНУТРИ эффекта или вычисления. Снаружи звать нечего — и
 * молчаливое «ничего не произошло» тут хуже отказа: подписка, которую
 * никто не снимет, живёт до перезагрузки страницы.
 */
export function onCleanup(fn: () => void): void {
  if (listener === null) {
    throw new Error(
      'onCleanup вне эффекта: снимать нечего и некому. ' +
        'Похоже, вызов уехал из эффекта наружу',
    )
  }
