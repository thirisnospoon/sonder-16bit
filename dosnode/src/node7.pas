...
{ Журнал ноды.
  ...
  запустилась». Файл — единственный способ сказать что-либо тому, кто
  снаружи.

  ФАЙЛ ЗАКРЫВАЕТСЯ ПОСЛЕ КАЖДОЙ СТРОКИ, и это не осторожность, а
  единственный способ быть услышанным. Держать его открытым и звать
  Flush ПРОБОВАЛИ: с точки зрения DOS всё правильно — RTL сбрасывает
  буфер, INT 21h пишет, — но DOSBox держит хостовый файл своим
  буфером и отдаёт его наружу лишь при закрытии. Снаружи журнал
  застывал на строках предыдущего запуска, а текущий выглядел немым.
  Мимо проверки это не прошло: сквозной след не нашёлся там, где ему
  полагалось быть.

  Цена — открытие файла на строку, то есть разбор пути и поиск по
  каталогу. При десятке команд в секунду (потолок линии) это доли
  процента от тех тринадцати миллисекунд, которые круговой обмен
  тратит в любом случае.

  Журнал растёт и не подрезается: нода живёт до перезапуска контейнера,
  а дописывание в конец переживает перезапуск — склеенная история важнее
  экономии. }
...
{ Имя операции для журнала.

  Обратное к KindOf. Писать в журнал число вместо имени значило бы
  заставлять читающего держать в голове порядок вариантов записи —
  порядок, который меняется при первом же добавлении операции. }
function OpName(K: TOpKind): string;
begin
  case K of
    opRegister: OpName := 'RegisterUser';
    opPost:     OpName := 'CreatePost';
    opComment:  OpName := 'CreateComment';
    opDelete:   OpName := 'DeletePost';
    opFollow:   OpName := 'FollowUser';
    opUnfollow: OpName := 'UnfollowUser';
    opBan:      OpName := 'BanUser';
    opPing:     OpName := 'Ping';
  else
    OpName := '?';
  end;
...
{ Сквозной след команды.

  Он рождается в оболочке на границе HTTP, уходит сюда в meta.traceId и
  здесь же ложится в журнал. По нему сходятся три мира: браузер знает
  его из заголовка ответа, оболочка пишет в каждой своей строке, ядро —
  в этой. Без него вопрос «что случилось С ЭТИМ запросом» решается
  сличением времён, а под эмулятором часы свои.

  Конверт мог не разобраться — тогда следа нет, и выдумывать его нельзя:
  придуманный идентификатор увёл бы ищущего в пустоту убедительнее, чем
  честный вопросительный знак. }
function TraceOf(C: PChanCtx): string;
var
  { Через переменную, а не через имя функции. В режиме TP чтение имени
    функции — это РЕКУРСИВНЫЙ ВЫЗОВ, а не чтение результата, и проверка
    «а не пусто ли» ушла бы в бесконечность. }
  R: string;
begin
  case C^.Kind of
    opRegister: R := StrHead(C^.RqReg.meta.traceId);
    opPost:     R := StrHead(C^.RqPost.meta.traceId);
    opComment:  R := StrHead(C^.RqComm.meta.traceId);
    opDelete:   R := StrHead(C^.RqDel.meta.traceId);
    opFollow:   R := StrHead(C^.RqFol.meta.traceId);
    opUnfollow: R := StrHead(C^.RqUnf.meta.traceId);
    opBan:      R := StrHead(C^.RqBan.meta.traceId);
  else
    { У пинга следа нет по контракту: он не команда, а вопрос о
      состоянии, и своей истории у него не бывает. }
    R := '';
  end;
  if R = '' then
    R := '?';
...
begin
  ...
  begin
    ...
    Inc(Malformed);
    Note('отказ trace=' + TraceOf(C) + ' причина=конверт');
  ...
  case C^.Kind of
    ...
    Inc(Refused);
    Note('отказ trace=' + TraceOf(C) + ' причина=операция');
    ReplyError(Chan, ERR_MALFORMED_ENVELOPE);
    Exit;
  end;
  ...
  begin
    ...
    Inc(Refused);
    Note('отказ trace=' + TraceOf(C) + ' причина=арена op=' + OpName(C^.Kind));
    ReplyError(Chan, ERR_DECIDER_PANIC);
    Exit;
  end;

  { След ПЕРЕД отправкой ответа, а не после. Отправка может не
    состояться — линия оборвана, кольцо переполнено, — и запись после
    неё потерялась бы ровно в том случае, ради которого журнал и
    ведётся. }
  if D.accepted then
...
begin
  ...
  begin
    ...
    { ЛИНИЯ МОЛЧИТ ДОЛЬШЕ СРОКА — ЗАВЕРШАЕМСЯ.
      
      Переподключиться нода не может: нульмодем эмулятора идёт на порт
      ОДИН РАЗ при старте и обратно не идёт. Перезапуск оболочки рвёт
      сокет, эмулятор при этом жив, цикл честно крутится, команд больше
      не будет никогда — а снаружи это выглядит как работающая система,
      отвечающая «ядро недоступно» на всё подряд.
      
      Выход отдаёт решение тому, кто его может принять: контейнер
      перезапускается политикой, ждёт порт и подключается заново.
      Состояния у ноды нет, команды в работе оболочка роняет отказом
      сама — терять тут нечего. }
    if PortDead(Tick) then
    begin
      Note('линия молчит дольше срока, выходим для перезапуска');
      Halt(0);
    end;
