...
begin
  if (Port[PortBase + RegLsr] and LsrTxEmpty) = 0 then
  begin
    RawWrite := False;
    Exit;
  end;
...
begin
  ...
  begin
    ...
    begin
      { Передатчик занят. Байт уже забран из кольца, и вернуть его
        нельзя — кольцо однонаправленное. Это потеря, и она считается
        как ошибка линии, а не замалчивается.

        На практике сюда попасть трудно: RawWrite отказывает, только
        если предыдущий байт ещё не ушёл, а мы проверяем это перед
        каждым. Но «трудно» не значит «никогда». }
      Inc(Stats.LineErrs);
      Break;
    end;
    Inc(Stats.TxBytes);
