...
const
  ...
  { Биение после готовности: раз в пять секунд.

    ЗАЧЕМ ОНО ВООБЩЕ. Молчание на исправной линии и молчание на мёртвой
    выглядят с этой стороны ОДИНАКОВО: гейтвей сам ничего не шлёт, а
    приветствие мы посылали только пока не получили ответа. Оболочка
    перезапустилась — сокет умер, эмулятор жив, нода честно крутит цикл
    и ждёт команд, которых больше не будет. Снаружи это выглядит как
    работающая система, отвечающая «ядро недоступно» на всё подряд.

    Приветствие раз в пять секунд стоит восьми байт: на линии 11 503 Б/с
    это семь десятитысячных её пропускной способности. }
  HeartbeatTicks = 91;

  { Через сколько молчания считать линию мёртвой.

    Тридцать секунд — шесть пропущенных биений подряд. Одно-два могут
    пропасть на занятой линии или потеряться в кольце; шесть подряд
...
{ Линия молчит дольше срока: другая сторона исчезла. }
function PortDead(Now: LongInt): Boolean;
...
{ Линия молчит дольше всякого разумного срока.

  Спрашивать об этом обязан тот, кто может что-то сделать: сам драйвер
  порта переподключиться не умеет — нульмодем эмулятора идёт на порт
  один раз при старте. Ответ «да» означает, что программе пора
  завершиться и дать себя перезапустить. }
function PortDead(Now: LongInt): Boolean;
begin
  { До первого обмена не судим: линия ещё поднимается. }
  PortDead := (State = psReady) and (LastRx > 0) and (Now - LastRx >= DeadTicks);
end;
...
begin
  ...
  { Пока не ответили — часто, после ответа — редким биением. Второе и
    отличает мёртвую линию от тихой. }
  if ((State = psAnnouncing) and (Now - LastHello >= HelloEveryTicks))
     or ((State = psReady) and (Now - LastHello >= HeartbeatTicks)) then
  begin
    SendHello;
    LastHello := Now;
  end;
