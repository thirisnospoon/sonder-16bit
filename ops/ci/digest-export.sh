...
# ВТОРОЕ МНЕНИЕ, добытое инструментом без понятия «символ».
#
# Проверка выше зелена потому, что в ней стоит `LC_ALL=C`. Забудь его —
# и она снова начнёт считать символы: ровно так она однажды объявила
# негодными 6971 запись из 6972 при исправной выгрузке. Проверяющий,
# делящий с проверяемым понятие «символ», разделит с ним и ошибку.
#
# Программа на Brainfuck такого понятия не имеет вовсе: она читает байт
# и прибавляет единицу. К тому же она смотрит, ГДЕ СТОЯТ переводы
# строк, а не только на длины строк, — файл, где одна запись короче, а
# другая длиннее, awk по длинам поймает, а вот делением размера на длину
# такой файл не поймать никак (ADR-0019).
bash ops/ci/reclen.sh "$TARGET" || exit 1
