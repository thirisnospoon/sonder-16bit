...
# Берётся модуль, ОБЪЯВЛЕННЫЙ недостижимым прямо сейчас. Здесь стоял
# TcLog — и перестал годиться в тот день, когда журнал ядра подключили к
# линии: модуль вошёл в программу, из списка ушёл, а фальсификация
# продолжила добавлять уже добавленное и зеленеть. Проверка, потерявшая
# способность падать, молчит ровно так же, как исправная.
replace_in "$NODE" "  DcdTypes, DcdSrv, DmDecide;" \
                   "  DcdTypes, DcdSrv, DmDecide, TcLoop;"
