...
# ВТОРОЙ ПРИГОВОР, и он важнее первого.
#
# У дренажа два способа проснуться: дверной звонок Firebird на вставку в
# outbox и опрос раз в секунду как запасной. Звонок только УКОРАЧИВАЕТ
# ожидание — без него система работает, просто в двадцать раз медленнее
# на этом пути, и молчит об этом.
#
# Измерено: со звонком медиана около 40 мс, без него — 860. Порог SLO в
# секунду эти режимы НЕ РАЗЛИЧАЕТ: 935 мс на выродившемся составе
# проверку проходили. То есть проверка была слепа ровно к той поломке,
# которую только она и могла заметить.
#
# Сравнивается МЕДИАНА, а не p99: на пятнадцати замерах p99 — это
# максимум, самая шумная величина, а разница режимов лежит в медиане и
# составляет двадцать раз. Четыреста миллисекунд дают десятикратный
# запас с одной стороны и двукратный с другой.
DOORBELL_P50_MS="${SONDER_LAG_DOORBELL_MS:-400}"
...
echo
echo "$MEASURED" | tr ' ' '\n' | grep -v '^$' | sort -n \
  | awk -v slo="$SLO_P99_MS" -v bell="$DOORBELL_P50_MS" '
  { v[NR] = $1; sum += $1 }
  END {
    n = NR
    ...
    printf("  медиана против звонка: порог %d мс\n", bell)
    printf("  макс %5d мс\n", v[n])
    printf("  сред %5.0f мс\n", sum / n)
    bad = 0
    if (p99 >= slo) {
    ...
    if (p50 >= bell) {
      printf("\nЗВОНОК ОЧЕРЕДИ НЕ РАБОТАЕТ: медиана %d мс при пороге %d.\n", p50, bell)
      printf("Дренаж просыпается по часам, а не по вставке в outbox.\n")
      printf("Система при этом исправна и медленнее в двадцать раз —\n")
      printf("именно поэтому порог SLO в секунду этого не ловит.\n")
      bad = 1
    }
    if (bad) { exit 1 }
    printf("\nSLO соблюдён, звонок очереди работает\n")
  }
'
